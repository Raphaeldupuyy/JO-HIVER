...
var son_drapeau;
var score_final;
function passerEntreportes (player, tuile) {
  // on enleve la piece
  score += 1;
  scoreText.setText ('score : '+ score);
  console.log(tuile.x);
  entreportes.removeTileAt(tuile.x, tuile.y);
  entreportes.removeTileAt(tuile.x-1,tuile.y);
  entreportes.removeTileAt(tuile.x-2, tuile.y);
  entreportes.removeTileAt(tuile.x+1,tuile.y);
  entreportes.removeTileAt(tuile.x+2,tuile.y);
}
...
export default class niveau2 extends Phaser.Scene {
  ...
  preload() {
   ...
   // this.load.add("font","src/assets/font.otf");
    ...
    this.load.tilemapTiledJSON("carte", "src/assets/map_geant.tmj");
  ...
  create() {
   ...
   son_drapeau = this.sound.add('drapeau');
    ...
    entreportes = carteDuNiveau.createLayer(
      "entreportes",
      tileset
    )
    ...
    arbre_decor.setTileIndexCallback([72, 83, 81, 70, 59, 60, 19, 48, 82, 7], toucherdDecor, this);
    copie.setTileIndexCallback([72, 83, 81, 70, 59, 60, 19, 48, 82, 7], ralentirDecor, this);
    entreportes.setTileIndexCallback(4,arrive, this);
    arbre_decor.setTileIndexCallback(27, retourMenu, this);

    entreportes.setTileIndexCallback(54,passerEntreportes, this);
    ...
    this.physics.add.collider(this.player, entreportes);

    this.physics.add.overlap(this.player,entreportes);
  ...
  update() {
    ...
    if (!jeuCommence2) {
      ...
      if (this.clavier.space.isDown && jeuCommence2 == false) {
        son_start.play();
        score = 0;
